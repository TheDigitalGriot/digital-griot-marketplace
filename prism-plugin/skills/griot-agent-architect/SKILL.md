---
name: griot-agent-architect
description: Use when creating, scaffolding, structuring, or validating plugins, skills, and agent architectures for Claude Code or Claude Cowork. The Griot gold standard for multi-surface Claude plugin and multi-agent system design. Covers the .claude-plugin/plugin.json + marketplace manifest, component organization (agents, skills, slash commands, hooks, MCP/LSP servers, channels), agent/command/hook frontmatter, the .local.md per-project settings pattern, portable paths, surface compatibility, bundled validator scripts, and development workflow. Use this whenever the user mentions building a plugin, a skill, a slash command, a hook, an MCP server, a marketplace, an agent architecture, or asks about plugin.json/SKILL.md structure — even if they don't say "plugin" explicitly. Formerly named cl-plugin-structure (that name still resolves as a deprecation alias).
version: 0.8.0
---
